...
import javax.swing.*;
    ...
    public class PokemonForm extends JInternalFrame {
        ...
        private JTextField txtId, txtNome, txtTipoPrimario, txtTipoSecundario, txtNivel, txtHpMaximo,txtHpAtual;
        ...
        private void carregarPokemonParaEdicao(int id) {
            try {
                Pokemon pokemon = controller.buscarPorID(id);
                if (pokemon != null) {
                    ...
                    txtHpAtual.setText(String.valueOf(pokemon.getHpAtual()));
        ...
        private void salvarPokemon() {
            try {
                ...
                System.out.println(pokemonIdParaEdicao);

                if (pokemonIdParaEdicao == null) {
                    Pokemon novo = new Pokemon();
                    System.out.println(novo);
                    novo.setNome(nome);
                    novo.setTipo_primario(tipoPrimario);
                    novo.setTipo_secundario(tipoSecundario);
                    novo.setNivel(nivel);
                    novo.setHpMaximo(hpMaximo);
                    novo.setHpAtual(hpAtual);
                    PokemonController po = new PokemonController();

                    po.cadastrarPokemon(novo);

                    JOptionPane.showMessageDialog(this, "Pokémon cadastrado com sucesso!");
                } else {
                    Pokemon novo = new Pokemon();
                    novo.setId_pokemon(pokemonIdParaEdicao);
                    System.out.println(novo.getId_pokemon());
                    novo.setNome(nome);
                    novo.setTipo_primario(tipoPrimario);
                    novo.setTipo_secundario(tipoSecundario);
                    novo.setNivel(nivel);
                    novo.setHpMaximo(hpMaximo);
                    novo.setHpAtual(hpAtual);
                    controller.atualizarPokemon(novo);
