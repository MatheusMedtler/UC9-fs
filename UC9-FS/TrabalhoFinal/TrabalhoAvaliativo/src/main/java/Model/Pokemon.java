package Model;

public class Pokemon {
    private int id;
    private Integer fk_id_treinador;
    private String nome;
    private String tipoPrimario;
    private String tipoSecundario; // Pode ser null
    private int nivel;
    private int hpMaximo;
    private int hpAtual;


    public Pokemon(int id, Integer fk_id_treinador, String nome, String tipoPrimario, String tipoSecundario, int nivel, int hpMaximo, int hpAtual) {
        this.id = id;
        this.fk_id_treinador = fk_id_treinador;
        this.nome = nome;
        this.tipoPrimario = tipoPrimario;
        this.tipoSecundario = tipoSecundario;
        // As validações são chamadas aqui pelos setters
        setNivel(nivel);
        setHpMaximo(hpMaximo);
        setHpAtual(hpAtual);
    }

    public Pokemon(String nome, String tipoPrimario, String tipoSecundario, int nivel, int hpMaximo, int hpAtual) {
        this.nome = nome;
        this.tipoPrimario = tipoPrimario;
        this.tipoSecundario = tipoSecundario;
        setNivel(nivel);
        setHpMaximo(hpMaximo);
        setHpAtual(hpAtual);
    }

    public Pokemon() {
    }


    // Getters
    public int getId() { return id; }
    public Integer getFk_id_treinador() { return fk_id_treinador; }
    public String getNome() { return nome; }
    public String getTipoPrimario() { return tipoPrimario; }
    public String getTipoSecundario() { return tipoSecundario; }
    public int getNivel() { return nivel; }
    public int getHpMaximo() { return hpMaximo; }
    public int getHpAtual() { return hpAtual; }

    // Setters com validação
    public void setId(int id) { this.id = id; }
    public void setFk_id_treinador(Integer fk_id_treinador) { this.fk_id_treinador = fk_id_treinador; }
    public void setNome(String nome) { this.nome = nome; }
    public void setTipoPrimario(String tipoPrimario) { this.tipoPrimario = tipoPrimario; }
    public void setTipoSecundario(String tipoSecundario) { this.tipoSecundario = tipoSecundario; }


    public void setNivel(int nivel) {
        if (nivel < 1 || nivel > 100) {
            throw new IllegalArgumentException("O Nível do Pokémon deve estar entre 1 e 100.");
        }
        this.nivel = nivel;
    }

    public void setHpMaximo(int hpMaximo) {
        if (hpMaximo <= 0) {
            throw new IllegalArgumentException("O HP Máximo do Pokémon deve ser maior que 0.");
        }
        this.hpMaximo = hpMaximo;
    }

    public void setHpAtual(int hpAtual) {
        if (hpAtual < 0) {
            throw new IllegalArgumentException("O HP Atual Ta errado.");
        }
        if (hpAtual > this.hpMaximo) {
            throw new IllegalArgumentException("O HP Atual não pode ser maior que o HP Máximo.");
        }
        this.hpAtual = hpAtual;
    }




    @Override
    public String toString() {
        return "Pokemon{" +
                "id=" + id +
                ", fk_id_treinador=" + fk_id_treinador +
                ", nome='" + nome + '\'' +
                ", tipoPrimario='" + tipoPrimario + '\'' +
                ", tipoSecundario='" + tipoSecundario + '\'' +
                ", nivel=" + nivel +
                ", hpMaximo=" + hpMaximo +
                ", hpAtual=" + hpAtual +
                '}';
    }
}